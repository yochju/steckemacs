...
#+PROPERTY: header :exports none
#+TBLNAME: keys
| C-c X   | Kill emacs (including the daemon if it is running) | General | (lambda () (interactive) (shell-command "pkill emacs"))                                      | global    |
| C-c s   | Open emacs shell                                   | General | 'shell                                                                                       | global    |
| C-S-l   | List available packages                            | General | 'package-list-packages-no-fetch                                                              | global    |
| C-c d   | Change dictionary                                  | General | 'ispell-change-dictionary                                                                    | global    |
| C-h C-f | Go to the definition of the function under cursor  | General | 'find-function-at-point                                                                      | global    |
| C-h C-v | Go to the definition of the variable under cursor  | General | 'find-variable-at-point                                                                      | global    |
| C-c C-w | Browse URL under cursor                            | General | 'browse-url-at-point                                                                         | global    |
| cd      |                                                    | General | (lambda () (interactive) (dired (file-name-directory (or load-file-name buffer-file-name)))) | key-chord |
| sb      |                                                    | General | 'speedbar                                                                                    | key-chord |
| cg      |                                                    | General | 'customize-group                                                                             | key-chord |
