...
*** TODO General
#+TBLNAME: keys
| C-c X   | Kill emacs (including the daemon if it is running) | General | (lambda () (interactive) (shell-command "pkill emacs"))                                       | global-set-key          |
| C-c s   | Open emacs shell                                   | General | 'shell                                                                                        | global-set-key          |
| C-S-l   | List available packages                            | General | 'package-list-packages-no-fetch                                                               | global-set-key          |
| C-c d   | Change dictionary                                  | General | 'ispell-change-dictionary                                                                     | global-set-key          |
| C-h C-f | Go to the definition of the function under cursor  | General | 'find-function-at-point                                                                       | global-set-key          |
| C-h C-v | Go to the definition of the variable under cursor  | General | 'find-variable-at-point                                                                       | global-set-key          |
| C-c C-w | Browse URL under cursor                            | General | 'browse-url-at-point                                                                          | global-set-key          |
| cd      |                                                    | General | (lambda () (interactive) (dired (file-name-directory (or load-file-name buffer-file-name))))  | key-chord-define-global |
| sb      |                                                    | General | 'speedbar                                                                                     | key-chord-define-global |
| cg      |                                                    | General | 'customize-group                                                                              | key-chord-define-global |

#+NAME: gen-keys
#+BEGIN_SRC emacs-lisp :var keys=keys :results output :tangle no :exports none
  (mapcar (lambda (l) 
            (let ((global (equal (car (last l)) "global-set-key"))) 
              (princ (format "(%s %s\"%s\"%s %s)\n" (car (last l)) (if global "(kbd" "") (car l) (if global ")" "") (nth 3 l))))
            ) 
          keys)
#+END_SRC
