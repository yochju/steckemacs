...
(defun my-configure-term ()
    ...
    (define-key term-raw-map (kbd "M-r") 'isearch-backward)
    (define-key term-raw-map (kbd "M-s") 'isearch-forward))
(add-hook 'term-mode-hook 'my-configure-term t)
#+END_SRC

I'd like to have the terminal respect the default keys of the shell as much as possible hence I remap the non-standard bindings to the defaults and the Emacs keys to meta-bindings.
