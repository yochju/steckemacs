...
| C-h n             | Org       | Open Org mode agenda                                      | (lambda () (interactive) (org-agenda nil "n"))                                                                                             |
...
| C-h C-u           | Org       | Open/switch to todo.org                                   | (lambda () (interactive) (find-file "~/org/todo.org"))                                                                                     |
...
Mode to color TODO/FIXME black on red.
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; helpers to make TODO state changes clock the time
  ...
  ;; enable sync mode if org-mobile-directory is set
...
#+BEGIN_SRC emacs-lisp
  (defun stk/disable-ac-for-emacs-lisp-in-org-mode ()
    (when (boundp 'org-mode-p) (set (make-local-variable 'ac-modes) (delq 'emacs-lisp-mode ac-modes))))
  (add-hook 'emacs-lisp-mode-hook 'stk/disable-ac-for-emacs-lisp-in-org-mode)
#+END_SRC

Org mode makes a mess with emacs lisp blocks and auto-complete so we try to disable in the hook.
