...
(defun stk/tks (ipos tables params)
  "Formatting function for org `clocktable' that generats TKS compatible output.
Usage example:
#+BEGIN: clocktable :scope agenda :block today :formatter stk/taxi
#+END: clocktable
"
  (let (tbl entry entries time)
    (while (setq tbl (pop tables))
	  ...
	  (while (setq entry (pop entries))
        (let* ((level (car entry))
               (headline (replace-regexp-in-string "^\\(DONE \\)\\|\\(IN PROGRESS \\)" "" (nth 1 entry)))
