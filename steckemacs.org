...
*steckemacs* is an Emacs configuration that integrates around 60 modes (packages). Stock Emacs is not very usable and lacks many convenient commands. Over the years I've discovered many good extensions and added them to this setup. The goal is to keep everything in one file =steckemacs.org= (plus =.emacs= for the initialization) and not as many others to split it over lots of files and directories. Packages are retrieved from the Emacs package repositories on the internet on the first startup.
...
#+BEGIN_QUOTE
Quelpa is your personal local Emacs Lisp Package Archive (ELPA) with packages built on-the-fly directly from source.
#+END_QUOTE

All the packaging solutions that existed annoyed me so I wrote my own and called it =quelpa=.

Given a recipe in [[https://github.com/milkypostman/melpa#recipe-format][melpa's format]], it gets the package source code, builds an elpa compatible package and installs that to the local package archive. Packages can be managed with =list-packages=.
...
#+BEGIN_SRC emacs-lisp
(quelpa '(shell-switcher :fetcher github :repo steckerhalter/shell-switcher :files ("rswitcher.el" "shell-switcher.el")))
...
I have finally started learning the =smartparens-bindings=. I also discovered a very interesting alternative called =lispy= (https://github.com/abo-abo/lispy). Let's see who can win me over :)
