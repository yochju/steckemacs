...
#+BEGIN_SRC emacs-lisp
  ...
  (defun stk/diff-hl-update-each-buffer ()
    (interactive)
    (mapc (lambda (buffer)
            (condition-case nil
  ...
  (add-hook 'git-commit-commit-hook 'stk/diff-hl-update-each-buffer)
#+END_SRC

Turn on the global mode and use the margin (not the fringe) to show the diff. Update each buffer after having commited something with Magit.
